...
export function Home(){
  ...
  async function handleSendImage(){
    ...
    //Estado que Pega a informação entregue pela API da Imagem em JSON
    const imageJSON = await response.data?.predictions.filter((teste: any) => teste.probability > 0.7);
    console.log(imageJSON);
    const pickedProps = (({ probabilidade, Documento }) => {
      return { probabilidade, Documento }
    })(imageJSON[0]);
    console.log(pickedProps);
    setInfoJson(pickedProps);
